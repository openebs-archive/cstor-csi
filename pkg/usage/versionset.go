...
import (
	...
	"github.com/openebs/cstor-csi/pkg/kubernetes/client"
	ns "github.com/openebs/cstor-csi/pkg/kubernetes/namespace"
...
func (v *VersionSet) fetchAndSetVersion() error {
	...
	k8s, err := client.GetServerVersion()
	if err != nil {
		return err
	}
	// eg. linux/amd64
	...
	v.nodeType, err = node.GetOSAndKernelVersion()
...
func getUUIDbyNS(namespace string) (string, error) {
	NSstruct, err := ns.NewKubeClient().Get(namespace, metav1.GetOptions{})
	if err != nil {
		return "", err
	}
	if NSstruct != nil {
