...
func (cs *controller) CreateVolume(
	...
	var (
		err    error
		jvcObj *apismaya.JivaVolumeClaim
	)
	...
	jvcObj, err = jvc.NewKubeclient().
		WithNamespace(utils.OpenEBSNamespace).
		Get(volName, metav1.GetOptions{})

	if err == nil && jvcObj != nil && jvcObj.DeletionTimestamp == nil {
		goto createVolumeResponse
	}

	jvcObj, err = jvc.NewBuilder().
		WithName(volName).
		WithNamespace(utils.OpenEBSNamespace).
		WithCapacity(size).
		WithReplicaCount(rCount).
		WithPhase(apismaya.JivaVolumeClaimPhasePending).
		Build()
	if err != nil {
...
func (cs *controller) DeleteVolume(
	...
	jObj := jiva.New(
		jiva.WithVolID(volumeID),
	)

	obj, err = jObj.GetVolume()

	jvc := obj.(*apismaya.JivaVolumeClaim)
	if jvc != nil && jvc.DeletionTimestamp != nil {
		goto deleteResponse
	}

	// Delete the corresponding CVC
	err = jObj.DeleteVolume()
	if err != nil {
