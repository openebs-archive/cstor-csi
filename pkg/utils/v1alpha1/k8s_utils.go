...
func DeleteOldCSIVolumeCR(vol *v1alpha1.CSIVolume, nodeID string) (err error) {
	openebsClient, _ := loadClientFromServiceAccount()
	listOptions := v1.ListOptions{
		// TODO Update this label selector name, name in camera gallary
		LabelSelector: "Volname=" + vol.Name,
	}

	csivols, err := openebsClient.OpenebsV1alpha1().CSIVolumes(OpenEBSNamespace).List(listOptions)
	// Add description over here that why multiple CRs can be there for one
	// volume
